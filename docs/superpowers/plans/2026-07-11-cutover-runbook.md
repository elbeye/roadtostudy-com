...
| Faz 1.5 Zamanlanmış yayın paritesi | ✅ WP'deki 1502 `future` post EmDash'te de planlı: `scripts/wp-schedule-future.mjs` (yeni) `status='scheduled'` + `scheduled_at`=WP `date_gmt` yazar (slug ile, yalnız draft'ken — idempotent). Prod D1 doğrulandı: 2246 published / **1502 scheduled** (2026-07-12T03:00Z → 2027-06-01T09:00Z) / 18 draft. Worker'ın dakikalık cron sweep'i her postu zamanında yayınlar. Not: seed pipeline'ı scheduled durumunu ifade edemiyor (EmDash seed apply kısıtı) — bu adım her tam import'tan sonra koşulmalı. |
